...
// fieldname writes s unquoted to e if safe to do so for object keys
func (e *encodeState) fieldname(s string) (int, error) {
	needQuote := false
	if len(s) > 0 {
		if s[0] >= '0' && s[0] <= '9' {
			needQuote = true
		}
		for i := 0; !needQuote && i < len(s); i++ {
			c := s[i]
