...
func TestSetVariableEval(t *testing.T) {
	test := &Test{
		Name: "Testname",
		Request: Request{
	...
	for i, tc := range []struct {
		...
		{"3 > 7 ? 4 : 6", "6"},
		{"3 <= 7 ? 4 : 6", "4"},
		{"Test.Result.Tries * 2 + 1", "11"},
		// {"'bar' IN Test.Response.Redirections", "true"},
		// {"'mik' IN Test.Response.Redirections", "false"},
		{"strlen('foobar')", "6"},
		{"substring('foobar',2,5)", "oba"},
		{"seconds(Test.Response.Duration) < 2.5 ? 'fast' : 'slow'", "fast"},
		{"seconds(Test.Response.Duration) > 1.0 ? 'okay' : 'astonishing'", "okay"},
		{"substring(Test.Request.URL,0,4)", "http"},
		{"strindex(Test.Request.URL,'example')", "11"},
		{"strindex(Test.Request.URL,'XXX')", "-1"},
		{"replace(Test.Name,'name','FOO')", "TestFOO"},
	} {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			sv := SetVariable{Eval: tc.expr}
			got, err := sv.Extract(test)
			if err != nil {
				t.Fatal(err)
			}
			if got != tc.want {
